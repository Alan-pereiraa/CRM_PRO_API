...
export class DashboardService {
    ...
    async getOverView(accountId: string) {
        ...
        const totalContacts = projects.reduce((acc, project) => acc + project.contacts.length, 0);
        const completedTasks = projects.reduce((acc, project) => acc + project.tasks.filter(task => task.status === 'COMPLETED').length, 0);
        const pendingTasks = projects.reduce((acc, project) => acc + project.tasks.filter(task => task.status === 'PENDING').length, 0);
